...
@Slf4j
@Component
public class RemoveOzonFlowStartCommand extends AbstractBotCommand {
    private final FlowRegistry flowRegistry;
    private final SessionManager sessions;

    public RemoveOzonFlowStartCommand(FlowIO io, FlowRegistry registry, SessionManager sessions) {
        super(io, "/remove_ozon", "Удалить товар из списка отслеживания", true);
    ...
    public void execute(Update update) {
        ...
        var flow = flowRegistry.create(RemoveOzonFlow.FLOW_ID)
                .orElseThrow(() -> new IllegalStateException("Flow not registered: " + RemoveOzonFlow.FLOW_ID));
        sessions.start(RemoveOzonFlow.FLOW_ID, chatId, userId, flow, 600);
    }
}