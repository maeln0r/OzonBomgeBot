...
public final class AskSkuState implements State {
    ...
    public Next enter(FlowContext ctx, FlowIO io) {
        var kb = FlowIO.Keyboard.builder().row(new FlowIO.Button("Отмена", cb(PriceHistoryFlow.FLOW_ID, "cancel"))).build();
    ...
    public Next onCallback(FlowContext ctx, FlowIO io, String data) {
        if (data.equals(cb(PriceHistoryFlow.FLOW_ID, "cancel"))) return new Next.Cancel("пользовательская отмена");
        return new Next.Stay();
    }
}
